...
from HOT.utils.quant_matmul import HQ_Conv2d, HQ_Linear, stoch_quantizer, norm_quantizer
...
class Mlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0.):
        ...
        self.fc1 = HQ_Linear(in_features, hidden_features, bias=True)
        self.act = act_layer()
        # self.fc2 = nn.Linear(hidden_features, out_features)
        self.fc2 = HQ_Linear(hidden_features, out_features, bias=True)
...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        self.qkv = HQ_Linear(dim, dim * 3, bias=qkv_bias)
        self.attn_drop = nn.Dropout(attn_drop)
        self.proj = HQ_Linear(dim, dim)
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, embed_dim=768):
        ...
        self.proj = HQ_Conv2d(in_chans, embed_dim, kernel_size=patch_size, stride=patch_size)
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        # Classifier head
        self.head = HQ_Linear(embed_dim, num_classes) if num_classes > 0 else nn.Identity()
