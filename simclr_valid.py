...
def main_worker(rank, run, parsed_args):    
    ...
    if args.MODEL.endswith('simclr'):
        model = timm.create_model('resnet50', pretrained=False, num_classes=dataset_class[args.DATASET])
    else:
    ...
    # get simclr_weight
    checkpoint = torch.load("./checkpoint/simclr_HLS.ckpt", map_location='cuda')

    # delete hadamard matrix in checkpoint
    for k in list(checkpoint.keys()):
        if k.endswith('hadamard_bs'):
            checkpoint[k] = torch.Tensor([[1]])
    ...
    # update the weight of R50 except fc layer
    ...
    train_dataset = dataset.get_mlp_dataset(args.DATASET, isTrain=True)
    test_dataset = dataset.get_mlp_dataset(args.DATASET, isTrain=False)
    
    if args.DISTRIBUTED:
        train_sampler = torch.utils.data.DistributedSampler(train_dataset, shuffle=True)
        train_loader = torch.utils.data.DataLoader(
            train_dataset, sampler=train_sampler, batch_size=int(args.BATCH_SIZE // GPU_NUM), shuffle=False,
            num_workers=args.WORKERS, pin_memory=True, drop_last=False)
        test_sampler = torch.utils.data.DistributedSampler(test_dataset, shuffle=True)
        test_loader = torch.utils.data.DataLoader(
            test_dataset, sampler=test_sampler, batch_size=int(args.BATCH_SIZE // GPU_NUM), shuffle=False,
            num_workers=args.WORKERS, pin_memory=True, drop_last=False)
    else:
        train_loader = torch.utils.data.DataLoader(
    ...
    optimizer = torch.optim.Adam(model.parameters(), args.LR, weight_decay=8e-4)
    ...
    for epoch in range(args.EPOCHS):
        ...
        train_loss, train_prec1, train_prec5= forward(
            epoch, train_loader, model, criterion, optimizer, training=True)
        
        if rank == 0:
            with torch.no_grad():
                val_loss, val_prec1, val_prec5= forward(
                    epoch, test_loader, model, criterion, optimizer, training=False)
...
def forward(epoch, data_loader, model, criterion, optimizer, training):
    if training:
        model.train()
    else:
        model.eval()
